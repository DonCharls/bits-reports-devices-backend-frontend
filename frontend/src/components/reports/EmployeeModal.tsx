...
import { Download, X as XIcon, Clock, Calendar, AlertTriangle, Edit2, CalendarSearch } from 'lucide-react';
...
export const EmployeeModal: React.FC<EmployeeModalProps> = ({
  ...
  // HR specific table sorting and state
  ...
  const hrTableRows = tableRows.filter(row => {
      ...
      <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
        <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col h-[600px] animate-in slide-in-from-bottom-8 ease-out duration-500">
          <div className="p-5 bg-red-600 text-white flex justify-between items-center shrink-0">
            ...
            <button onClick={() => { onClose(); setLogSearchDate(""); }} className="p-2 hover:bg-red-700 rounded-full transition-colors outline-none">
              <XIcon size={20} />
            </button>
          </div>
          ...
          <div className="p-8 space-y-6 flex-1 overflow-y-auto">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {logSearchDate && (<button onClick={() => setLogSearchDate("")} className="absolute -right-8 top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-500"><XIcon size={12} /></button>)}
              </div>
            </div>
