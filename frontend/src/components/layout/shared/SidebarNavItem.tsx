  ...
  return (
    <li className="relative" style={{ padding: '0 0 0 16px', overflow: 'visible' }}>
      <Link
        href={href}
        onClick={onClick}
        className={`flex items-center gap-4 py-3 relative z-10 ${active ? 'text-brand' : 'text-white/60 hover:text-white'}`}
        style={{ paddingLeft: '12px', paddingRight: collapsed ? '12px' : '24px' }}
        title={collapsed ? label : undefined}
      >
        <Icon size={22} className={`shrink-0 ${active ? 'text-brand' : 'text-white'}`} />
