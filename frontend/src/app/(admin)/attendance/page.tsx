...
function AttendanceContent() {
  ...
  useEffect(() => {
    ...
    if (branchQuery) {
      if (branchQuery === 'all') setActiveBranchId('all')
      else if (!isNaN(parseInt(branchQuery))) setActiveBranchId(parseInt(branchQuery))
    }
    if (statusQuery) {
      const s = statusQuery.toLowerCase()
      setSelectedStatus(s === 'present' ? 'present' : s === 'late' ? 'late' : s === 'absent' ? 'absent' : 'all')
    }
  }, [searchParams])
            ...
            <table className="w-full text-left">
              <thead className="bg-secondary/50 backdrop-blur-sm">
                <tr className="border-b border-border bg-secondary/50 backdrop-blur-sm">
                  ...
                  <th className="px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-tight text-right">Actions</th>
                </tr>
              </thead>
